...
prisonPopRace <- veraDataCounty %>%
  group_by(year) %>%
  mutate(
    year_aapi_JoP_pop = (sum(aapi_jail_pop, na.rm = TRUE) + sum(aapi_prison_pop, na.rm = TRUE)) / (sum(aapi_pop_15to64, na.rm = TRUE)) * 100,
...
incarceratedPopProp <- ggplot(prisonPopRace, aes(x = year)) +
  ...
  labs(
    title = "Percentage of US Population in Prison or Jail, by Race",
    x = "Year",
...
prisonPopGenPop <- veraDataCounty %>%
  group_by(year) %>%
  mutate(
    year_gen_pop = sum(total_pop, na.rm = TRUE),
...
prisonPopGenPopGraph <- ggplot(prisonPopGenPop, aes(x = year_gen_pop)) +
  ...
  labs(
    title = "US General Population vs. Incarcerated Population Between 1970 and 2018",
...
blackIncPop <- veraDataCounty %>%
  group_by(state) %>%
  mutate(
    state_black_prison_pop = (sum(black_jail_pop, na.rm = TRUE) + sum(black_prison_pop, na.rm = TRUE)),
